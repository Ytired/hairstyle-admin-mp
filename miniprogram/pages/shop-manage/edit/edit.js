...
Page({
  ...
  onTimeTap(e) {
		...
		const newTimes = [...this.data.times]
		const activeKey = this.data.activeKey
		const times = newTimes[activeKey][this.data.dates[activeKey].dateTimeStr]
		if (times[index]) {
			times[index] = null
		} else {
			times[index] = this.data.timeList[index]
		}

		newTimes[activeKey][this.data.dates[activeKey].dateTimeStr] = times
		console.log(times);
    this.setData({
    	times: newTimes,
    });
  ...
  takeoverTap(e) {
    const options = {
      lunar: false, // 在配置showLunar为false, 但需返回农历信息。足以次配置需要引入 农历插件
    };
		const selectedDay = this.calendar.getSelectedDates(options);
		let newDates = [];
		let newTimes = []
    for (let i = 0; i < selectedDay.length; i++) {
      const element = selectedDay[i];
      const dateTimeStr = joinStr(element);
      element.dateTimeStr = dateTimeStr;
			newDates.push(element);
			const time = this.hasTime(dateTimeStr)
			if (time) {
				newTimes.push({
    ...
    console.log("takeoverTap", newTimes); // => { year: 2019, month: 12, date: 3, ...}
	},
	hasTime(dateTimeStr) {
		const times = this.data.times
		for (let j = 0; j < times.length; j++) {
			const time = times[j];
			if (time[dateTimeStr] === dateTimeStr) return time;
		}
  ...
  initCalendar(e) {
    ...
    if (this.data.dates.length) {
      calendar.setSelectedDates(this.data.dates);
    }
    // calendar.enableDates(['2023-06-05'])
  },
  async onPopupShow() {
    let valueList = [];
    const res = await projectList.where({ shopID: this.data.id }).get();
    if (this.data.hID) {
      const valueRes = await connHairProject
        .where({
          hID: this.data.hID,
        })
        .get();
  ...
  async saveHairstylistInfo() {
    ...
    if (hID) {
      ...
      const all = this.data.checkboxValueList.map((item) =>
        this.addProject({ hID, pID: item, shopID: id })
      );
      ...
      hairstyList.add({
        ...
        success: (res) => {
          this.data.checkboxValueList.map((item) =>
            this.addProject({ hID: res._id, pID: item, shopID: id })
          );
          hairstyList
            .where({
              shopID: id,
